...
class Classify extends XY_Controller
{
    ...
    public function submit()
    {
        try {
            ...
            if ($id) {
                $result = $this->common->update('mall_classify', array('id' => $id), $values);
            } else {
                $result = $this->common->insert('mall_classify', $values);
            }
    ...
    public function delete()
    {
        try {
            ...
            // 检查是否含有下级
            $has_next = $this->common->count_all_results('mall_classify', array('parent_id' => $id));
            if ($has_next > 0) {
                throw new Exception('已被使用，禁止删除', '1');
            }
            $result = $this->common->delete('mall_classify', array('id' => $id));
            $this->ajax_output->output('200', null, $result);
        } catch (Exception $e) {
            $this->ajax_output->output($e->getCode(), $e->getMessage());
        }
